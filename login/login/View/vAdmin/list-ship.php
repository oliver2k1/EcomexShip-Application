<?PHP 
  ...
  include '../../Dungchung/dbcon.php';
?>
          <div class="content-wrapper">
            <div class="container-xxl flex-grow-1 container-p-y">
             <div class="card">
                <h5 class="card-header">Đơn hàng đang giao phát</h5>
                <?php
                ...
                ?>
                  <?php
                      ...
                      $count_query = "SELECT COUNT(id) AS total FROM oder WHERE `status` = 1 AND `trangthai` = 5";
                      ...
                      $order_result = $order->show_order_ship($offset, $per_page);
                    ?>
                      ...
                      ?>
                        ...
                        <td>
                          <?php
                            ...
                            $selectQuery = "SELECT * FROM `pdf_data` WHERE `idOrder` = '$idOrder'";
                            $squery = mysqli_query($con, $selectQuery);
                            $result_pdf = mysqli_fetch_assoc($squery);
                            if ($result_pdf) {
                              echo '<a href="../../Dungchung/pdf/'.$result_pdf['filename'].'" class="btn btn-info" download>Tải xuống</a>';
                            } else {
                              echo "<span class='badge bg-label-secondary me-1'>Chưa có label</span>";
                            }
                          ?>
                        <td>
                          <div class="dropdown">
                            ...
                            <a class="dropdown-item" onclick="return confirm('Bạn có chắc chắn muốn xóa đơn hàng này?')" href="list-transporting.php?idOrder=<?php echo $result['idOrder'] ?>"><i class="bx bx-trash me-1"></i> Xóa đơn hàng</a>
                            </div>
                          </div>
                        </td>
                        </tr>
                ...
                <?php
                  ...
                  for ($i = $start_page; $i <= $end_page; $i++) {
                    if ($i == $current_page) {
                      ...
                      echo "<li class='page-item'><a class='page-link' href='list-ship.php?page=".$i."'>".$i."</a></li>";
                    }
                  }
                  echo "</ul>";
                  echo "</nav>";
