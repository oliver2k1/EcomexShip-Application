...
?>
          <div class="content-wrapper">
            ...
            <form action="../../Dungchung/payment.php?id_user=<?php echo $_SESSION["id"] ?>" method="post" target="_blank">
             <div class="card">
                <h5 class="card-header">Đơn hàng đang giao phát <button type="submit" name="submit" class="btn btn-primary"> Thanh toán</button></h5>
                <?php
                  // Số lượng đơn hàng hiển thị trên một trang
                  $per_page = 10;
                  // Tính tổng số đơn hàng
                  $name_user = $_SESSION['name'];
                  $count_query = "SELECT COUNT(id) AS total FROM oder WHERE `status` = 1 AND `trangthai` = 5 AND `name_user` = '$name_user'";
                  ...
                  $order_result = $order->show_order_ship_customer($offset, $per_page,$name_user);
                ?>
