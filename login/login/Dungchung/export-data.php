...
$sql = "SELECT * FROM `oder` WHERE `time` BETWEEN '$start_date' AND '$end_date' AND `status` = 1 order by `time` DESC";
...
if ($result) {
  ...
  if ($result->num_rows > 0) {
    ...
    header('Content-Disposition: attachment;filename="'.$start_date.' to '.$end_date.'.xlsx"');
